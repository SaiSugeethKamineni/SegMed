...
def get_data_testing(test_images_file, test_gt_file, n_test_images, patch_height, patch_width, channel, config):
    ...
    test_imgs = test_imgs_original[0: n_test_images, :, :, :]
...
def recompose_overlap(preds, img_h, img_w, stride_h, stride_w, channel, loss_weight=None):
    ...
    for i in range(N_full_imgs):
        for h in range((img_h - patch_h) // stride_h + 1):
            for w in range((img_w - patch_w) // stride_w + 1):
                full_prob[i, :, int(h * stride_h + top):int((h * stride_h) + bottom),
                          int(w * stride_w + left):int((w * stride_w) + right)] += \
                    preds[int(k), :, int(top):int(bottom), int(left):int(right)]*weight
                full_sum[i, :, int(h * stride_h + top):int((h * stride_h) + bottom),
                         int(w * stride_w + left):int((w * stride_w) + right)] += weight
                k += 1
