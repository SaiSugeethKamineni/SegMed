...
def plot_boundary(image, true_masks=None, pred_masks=None, ax=None):
    ...
    if true_masks is not None:
        for i in range(true_masks.shape[-1]):
            contours = find_contours(true_masks[..., i], 0.5, fully_connected='high', positive_orientation='high')
            # print(np.shape(contours),contours)
...
def plot_train(train_path='stage1_train/'):
    """
...
def eval_n_plot_val(model, config, dataset_val, save_plots=False):
    ...
    for mm, image_id in tqdm(enumerate(image_ids)):
        ...
        # print('hi',img_name,image_id,gt_class_id ,gt_mask.shape)
        ...
        # print("---------------------------")
        # print(pred_box.shape, pred_class_id.shape, pred_score.shape, pred_mask.shape)
        # print(pred_score)
        # print("---------------------------")
        gt_box = utils.extract_bboxes(gt_mask)
        # Compute IoU scores for ground truth and predictions
        # print(gt_box,gt_mask.shape)
        # print("-------------------------")
...
def get_contours(path,image_id,image_type):
    """ Detects the nuclei in ground truth images.
    """
    if image_type =="pred":
        mask_ = cv2.imread(path +'/Pred/' + image_id, 0)
    else:
        mask_ = cv2.imread(path +'/Gt/' + image_id, 0)
    ...
    contours = find_contours(mask[..., 0], 0.5, fully_connected='high', positive_orientation='high')
    final = np.zeros((mask.shape[0],mask.shape[1],len(contours)))
    for i in range(final.shape[2]):
        for contour in contours[i]:
            final[int(contour[0]),int(contour[1]),i] = 1
    
    # Class ids: all ones since all are foreground objects
    class_ids = np.ones(final.shape[2])


    return final.astype(np.uint8),class_ids.astype(np.int8)
def create_masks(path,image_lists, save_plots=False):
    #creates mask layers based on contours 
    scores = []
    image_ids = image_lists

    for image_id in image_ids:
        # Load image and ground truth data
        gt_mask,gt_class_id =get_contours(path,image_id,"gt")
        pred_mask,pred_class_id =get_contours(path,image_id,"pred")
        # image = cv2.imread(path +'/Pred/' + image_id, 0)
        # print(pred_mask.shape,gt_mask.shape)
        # print("-------------------------")
        # print("pppkpkd",gt_class_id.shape,pred_class_id.shape )
        # print(pred_mask.shape,gt_mask.shape)
        # print("-------------------------")        
        gt_box = utils.extract_bboxes(gt_mask)
        pred_box = utils.extract_bboxes(pred_mask)
        pred_score = np.ones(pred_class_id.shape[0])
        # fig = plt.figure()
        # gs = gridspec.GridSpec(1, 1)
        # plot_boundary(image, true_masks=gt_mask, pred_masks=pred_mask,
        #               ax=fig.add_subplot(gs[0]))
    
        # fig.savefig('../images/validation_'+'.png', bbox_inches='tight')
        # plt.close()
        # print("gt_box,pred_box",gt_box.shape,pred_box.shape)
        # print(pred_mask.shape,gt_mask.shape)
        # print("-------------------------")
        # Compute IoU scores for ground truth and predictions
       
        # iou = utils.compute_ap_range(gt_box, gt_class_id, gt_mask,
        #                               pred_box, pred_class_id, pred_class_id, pred_mask,
        #                               iou_thresholds=None, verbose=0)
        # print(gt_box.shape, gt_class_id.shape, gt_mask.shape,pred_box.shape, pred_class_id.shape, pred_score.shape, pred_mask.shape)
...
def pred_n_plot_test(model, config, test_path='stage2_test_final/', save_plots=False):
    """
    ...
    fname =  test_path.split('/')[-2]+'_submission.csv'
